...
class DatabaseFile:
    def __init__(self):
        ...
        self.dbCursor.execute("CREATE TABLE IF NOT EXISTS user_record (id integer PRIMARY KEY, fName text, lName text, dob text, mob text, yob text, gender text, address text, phoneNum text, emailId text, indoorGame text, OutdoorGame text)")
...
#to update data in the database
    def Update(self, id, fName, lName, dob, mob, yob, gender, address, phoneNum, emailId, indoorGame, outdoorGame):
        self.dbCursor.execute(
            "UPDATE user_record SET fName = ?, lName = ?, dob = ?, mob = ?, yob = ?, gender = ?, address = ?, phoneNum = ?, emailId = ?, indoorGame = ?, outdoorGame = ? WHERE id = ?",
            (id, fName, lName, dob, mob, yob, gender, address, phoneNum, emailId, indoorGame, outdoorGame))
        self.dbConnection.commit()
